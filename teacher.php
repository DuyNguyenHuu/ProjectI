...
$sql = "SELECT * FROM `giaovien` NATURAL JOIN `gv-mh` NATURAL JOIN `monhoc` ";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
        ...
        echo "<div id = 'info'>";
            echo "<div><img src = ". $row["ANHTHE"] .">"."</div>";
            echo "<div id = 'inner-info'><Tên giáo viên: " . $row["TENGV"]."<br>";
            echo "Mã giáo viên: " . $row["MAGV"]."<br>";
            echo "Email: " . $row["EMAIL"]."<br>";
            echo "Số điện thoại: " . $row["SDT"]."<br>";
            echo "Quê quán: " . $row["QUEQUAN"]."<br>";
            echo "Môn học: " . $row["TENMONHOC"]."<br></div>";
        echo "</div>";
    }
} else {
    echo "0 results";
